...
for ((i=1; i<=$1; i++)); do
    ...
    2. Check that the types check via bun typecheck, the lint passes with bun lint, and the unit tests pass via bun run test. If any of these fail, fix the issue before moving on. \
